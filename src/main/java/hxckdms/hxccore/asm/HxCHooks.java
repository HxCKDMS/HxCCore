...
public class HxCHooks {
    public static IChatComponent onEmoteEvent(ICommandSender sender, IChatComponent component) {
        EmoteEvent event = new EmoteEvent(sender, component);

        if (MinecraftForge.EVENT_BUS.post(event)) return null;
        else if(component == event.getComponent()) return new ChatComponentTranslation("chat.type.emote", sender.getCommandSenderName(), component);
        else return event.getComponent();
    }

    public static boolean onLivingSwingEvent(EntityLivingBase entityLiving, ItemStack itemStack) {
        return MinecraftForge.EVENT_BUS.post(new LivingSwingEvent(entityLiving, itemStack));
    }
    ...
    public static int getXPCoolDown() {
        return Math.max(0, Integer.parseInt(GlobalVariables.server.getEntityWorld().getGameRules().getGameRuleStringValue("XPPickupCoolDown")));
    }
}
