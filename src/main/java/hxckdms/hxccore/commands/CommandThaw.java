...
public class CommandThaw extends AbstractSubCommand<CommandHxC> {
    ...
    public void execute(ICommandSender sender, LinkedList<String> args) throws CommandException {
        ...
        final int r = CommandBase.parseInt(args.get(0));
        final Predicate<BlockPos> predicate = pos -> player.worldObj.getBlockState(pos).getBlock() == Blocks.ICE || player.worldObj.getBlockState(pos).getBlock() == Blocks.SNOW_LAYER;
        ...
        StreamSupport.stream(BlockPos.getAllInBoxMutable(new BlockPos(player.posX - r, player.posY - r, player.posZ - r), new BlockPos(player.posX + r, player.posY + r, player.posZ + r)).spliterator(), false).filter(predicate).forEach(pos -> {
            if (player.worldObj.getBlockState(pos).getBlock() == Blocks.ICE) player.worldObj.setBlockState(pos, Blocks.WATER.getDefaultState(), 3);
            else if (player.worldObj.getBlockState(pos).getBlock() == Blocks.SNOW_LAYER) player.worldObj.setBlockToAir(pos);
        });
    }
