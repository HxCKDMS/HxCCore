...
public class CommandThaw extends AbstractSubCommand<CommandHxC> {
    ...
    public void execute(ICommandSender sender, LinkedList<String> args) throws CommandException {
        ...
        for (int x = -r; x <= r; x++) {
            for (int y = -r; y <= r; y++) {
                for (int z = -r; z <= r; z++) {
                    if (player.worldObj.getBlock((int) player.posX + x, (int) player.posY + y, (int) player.posZ + z) == Blocks.ice)
                        player.worldObj.setBlock((int) player.posX + x, (int) player.posY + y, (int) player.posZ + z, Blocks.water, 0, 3);
                    else if (player.worldObj.getBlock((int) player.posX + x, (int) player.posY + y, (int) player.posZ + z) == Blocks.snow_layer)
