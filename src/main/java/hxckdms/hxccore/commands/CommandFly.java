        ...
        switch (args.size()) {
            case 0:
                if (sender instanceof EntityPlayerMP) {
                    ...
                    sender.addChatMessage(ServerTranslationHelper.getTranslation(player, player.capabilities.allowFlying ? "commands.fly.self.enabled" : "commands.fly.self.disabled").setChatStyle(new ChatStyle().setColor(player.capabilities.allowFlying ? EnumChatFormatting.GREEN : EnumChatFormatting.YELLOW)));
                }
                break;
            case 1:
                EntityPlayerMP target = CommandBase.getPlayer(sender, args.get(0));
                sender.addChatMessage(ServerTranslationHelper.getTranslation(sender, target.capabilities.allowFlying ? "commands.fly.other.sender.enabled" : "commands.fly.other.sender.disabled", sender.getCommandSenderName()).setChatStyle(new ChatStyle().setColor(EnumChatFormatting.GRAY)));
                target.addChatMessage(ServerTranslationHelper.getTranslation(target, target.capabilities.allowFlying ? "commands.fly.other.target.enabled" : "commands.fly.other.target.disabled", target.getDisplayName()).setChatStyle(new ChatStyle().setColor(target.capabilities.allowFlying ? EnumChatFormatting.GOLD : EnumChatFormatting.RED)));
                break;
        }
    }
