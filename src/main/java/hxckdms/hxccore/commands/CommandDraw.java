...
public class CommandDraw extends AbstractSubCommand<CommandHxC> {
    ...
    public void execute(ICommandSender sender, LinkedList<String> args) throws CommandException {
        ...
        int posY = (int) Math.round(CommandBase.func_110666_a(sender, sender.getPlayerCoordinates().posY, args.get(2)));
        int posZ = (int) Math.round(CommandBase.func_110666_a(sender, sender.getPlayerCoordinates().posZ, args.get(3)));
        ...
        switch (args.get(0).toLowerCase()) {
            case "circle":
                ...
                sender.addChatMessage(ServerTranslationHelper.getTranslation(sender, "commands.draw." + (hollow ? "hollow" : "filled") + ".circle", posX, posY, posZ, stack.getDisplayName(), radius).setChatStyle(new ChatStyle().setColor(EnumChatFormatting.BLUE)));
                break;
            case "sphere":
                WorldHelper.draw3DEllipsoid(sender.getEntityWorld(), posX, posY, posZ, block, metadata, radius, hollow, precision, 2);
                sender.addChatMessage(ServerTranslationHelper.getTranslation(sender, "commands.draw." + (hollow ? "hollow" : "filled") + ".sphere", posX, posY, posZ, stack.getDisplayName(), radius).setChatStyle(new ChatStyle().setColor(EnumChatFormatting.BLUE)));
                break;
            case "2dsquircle":
                WorldHelper.draw2DEllipsoid(sender.getEntityWorld(), posX, posY, posZ, block, metadata, radius, hollow, precision, 4);
                sender.addChatMessage(ServerTranslationHelper.getTranslation(sender, "commands.draw." + (hollow ? "hollow" : "filled") + ".2DSquircle", posX, posY, posZ, stack.getDisplayName(), radius).setChatStyle(new ChatStyle().setColor(EnumChatFormatting.BLUE)));
                break;
            case "3dsquircle":
                WorldHelper.draw3DEllipsoid(sender.getEntityWorld(), posX, posY, posZ, block, metadata, radius, hollow, precision, 4);
                sender.addChatMessage(ServerTranslationHelper.getTranslation(sender, "commands.draw." + (hollow ? "hollow" : "filled") + ".3DSquircle", posX, posY, posZ, stack.getDisplayName(), radius).setChatStyle(new ChatStyle().setColor(EnumChatFormatting.BLUE)));
                break;
        }
    }
    ...
    public List<String> addTabCompletionOptions(ICommandSender sender, LinkedList<String> args) {
        ...
        else if (args.size() == 5) return CommandBase.getListOfStringsFromIterableMatchingLastWord(args.toArray(new String[args.size()]), Block.blockRegistry.getKeys());
