...
public class PermissionHandler {
    public static boolean canUseCommand(ICommandSender sender, ICommand command) {
        ...
        return getPermissionLevel(sender) == -1 || (commandOptional.isPresent() ? getPermissionLevel(sender) >= commandOptional.get().getValue() : sender.canUseCommand(permLevel, command.getName()));
    }
