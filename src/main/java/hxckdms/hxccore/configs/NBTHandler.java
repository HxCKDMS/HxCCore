...
public class NBTHandler  implements ITypeHandler, ICollectionsHandler {
    ...
    public void write(Field field, LinkedHashMap<String, LinkedHashMap<String, Object>> config, HxCConfig HxCConfigClass) throws IllegalAccessException {
        ...
        switch (NBTBase.NBTTypes[base.getId()]) {
            case "STRING":
                categoryValues.putIfAbsent(field.getName(), "STR:" + ((NBTTagString) base).func_150285_a_());
            case "INT":
                categoryValues.putIfAbsent(field.getName(), "I:" + ((NBTTagInt) base).func_150287_d());
            case "DOUBLE":
                categoryValues.putIfAbsent(field.getName(), "D:" + ((NBTTagDouble) base).func_150286_g());
            case "FLOAT":
                categoryValues.putIfAbsent(field.getName(), "F:" + ((NBTTagFloat) base).func_150288_h());
            case "SHORT":
                categoryValues.putIfAbsent(field.getName(), "S:" + ((NBTTagShort) base).func_150289_e());
            case "LONG":
                categoryValues.putIfAbsent(field.getName(), "L:" + ((NBTTagLong) base).func_150291_c());
            case "BYTE":
                categoryValues.putIfAbsent(field.getName(), "B:" + ((NBTTagByte) base).func_150290_f());
            case "COMPOUND":
            ...
            case "LIST":
                ...
                for (int i = 0; i < tagList.tagCount(); i++) tagListBuilder.append('\n').append(cHandler.writeInCollection(field, tagList.tagList.get(i), null, null, HxCConfigClass).stream().map(str -> "\t\t" + str).reduce((a, b) -> a + "\n" + b).orElse(""));
                tagListBuilder.append('\n').append('\t').append(']');
    ...
    public List<String> writeInCollection(Field field, Object value, HashMap<String, Object> subDataWatcher, ParameterizedType parameterizedType, HxCConfig HxCConfigClass) {
        ...
        switch (NBTBase.NBTTypes[base.getId()]) {
            ...
            case "LIST":
                ...
                lines.add("LIST:[");
                for (int i = 0; i < tagList.tagCount(); i++) lines.addAll(cHandler.writeInCollection(field, tagList.tagList.get(i), null, null, HxCConfigClass).stream().map(str -> "\t" + str).collect(Collectors.toList()));
                lines.add("]");
                return lines;
            default:
